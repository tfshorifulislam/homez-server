...
async function run() {
    try {
        ...
        app.get("/api/wishlist/properties/:email", async (req, res) => {
            const { email } = req.params;
            const wishlist = await wishlistCollection.find({
                userEmail: email,
            }).toArray();
            const propertyIds = wishlist.map((item) => new mongodb_1.ObjectId(item.propertyId));
        ...
        // Accept Property
        app.patch("/api/property/accept/:id", async (req, res) => {
            const id = req.params.id;
        ...
        app.delete("/api/property/reject/:id", async (req, res) => {
            const id = req.params.id;
